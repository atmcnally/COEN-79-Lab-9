...
#pragma mark - Toolkit Function Implementation
	...
	So, how do we delete the largest item from the left subtree, and place this same item at the root? 
	We can use our own bst_remove_max function with the call:
	bst_remove_max(root_ptr->left( ), root_ptr->data( ));
	Now you know why we proposed the bst_remove_max function. Used in this way, 
	it removes the largest item from the tree with root_ptr->left( ) as its root pointer, 
	and it places this largest item in the root’s data field.*/
    
    template <class Item>
    typename bag<Item>::size_type bst_remove_all (binary_tree_node<Item>*& root_ptr, const Item& target)
    // Precondition: root_ptr is a root pointer of a binary search tree (or may be NULL for an empty tree).
    // Postcondition: All copies of target have been removed from the tree and root_ptr now points to the root of the new
    // (smaller) binary search tree. The return value tells how many copies of the target were removed.
    {
        binary_tree_node<Item> *oldroot_ptr;
        ...
        // maximum item in our left subtree. We then continue searching for more copies of the target to remove.
...
#pragma mark - Member Function Implementation
    ...
    // NOTE: The copy constructor needs to make a new copy of the source’s tree, and point root_ptr to the root of this copy
    ...
    // is separate for the binary search tree of the bag that activated this method.
